  ...
  if (!_.isEmpty(request.route.settings.plugins[internals.pluginName])) {
    ...
    if (!_.isEmpty(requiredPermissions)) {
      ...
      await internals.permissionsFunc(request, h,function(
        ...
        let hasPermission = internals.checkPermissions(
          requiredPermissions,
          userPermissions
        );
        if (hasPermission) {
...
internals.checkPermissions = function(requiredPermissions, userPermissions) {
  ...
  _.forEach(requiredPermissions, function(requiredPermission) {
    ...
    Hoek.assert(
      parts.length === 2,
      "permission must be formatted: [routeName]:[read|create|edit|delete]"
    );
    ...
    if (
      _.isUndefined(userPermissions[routeName]) ||
      _.isUndefined(userPermissions[routeName][crud])
    ) {
